                ...
                <!-- Total Revenue -->
                  ...
                  <div class="card">
                    <div class="row row-bordered g-0">
                      <div class="col-md-12">
                        <h3 class="card-header m-0 me-2 pb-3"><strong>จัดการการรับโอนครุภัณฑ์</strong></h3>
...
$countQuery = "SELECT COUNT(*) AS total FROM `receive_transfer`";
...
$sort = $_GET['sort'] ?? 'oldest';
              $orderBy = match ($sort) {
                'name_asc' => 'ORDER BY eq.transfer ASC',
                'name_desc' => 'ORDER BY eq.transfer DESC',
                'latest' => 'ORDER BY eq.tran_id DESC',
                default => 'ORDER BY eq.tran_id ASC'
              };
              
              // นับข้อมูลทั้งหมด (ไว้ใช้กับ pagination)
              $countSQL = "SELECT COUNT(*) AS total FROM receive_transfer eq";
  ...
  $sql = "SELECT DISTINCT eq.* FROM receive_transfer eq $orderBy LIMIT $limit OFFSET $offset";
...
?>
	...
	<td align="center"><div class="bootstrap-scope"><a href="edittran.php?eid=<?php echo $row['tran_id'];?>">
	  <button type="button" class="btn btn-warning">แก้ไข</button>
	</a>
		<button type="button" class="btn btn-danger" data-bs-toggle="modal" data-bs-target="#exampleModal<?php echo $row['tran_id'];?>">
    ลบ
  </button>

  <!-- Modal -->
  <div class="modal fade" id="exampleModal<?php echo $row['tran_id'];?>" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <div class="modal-footer">
          ...
          <a href="actions/deltran.php?eid=<?php echo $row['tran_id'];?>">
