                ...
                <!-- Total Revenue -->
                  ...
                  <div class="card">
                    <div class="row row-bordered g-0">
                      <div class="col-md-12">
                        <h3 class="card-header m-0 me-2 pb-3"><strong>จัดการประเภทครุภัณฑ์</strong></h3>
...
$countQuery = "SELECT COUNT(*) AS total FROM `type`";
...
$sort = $_GET['sort'] ?? 'oldest';
              $orderBy = match ($sort) {
                'name_asc' => 'ORDER BY eq.type ASC',
                'name_desc' => 'ORDER BY eq.type DESC',
                'latest' => 'ORDER BY eq.type_id DESC',
                default => 'ORDER BY eq.type_id ASC'
              };
              
              // นับข้อมูลทั้งหมด (ไว้ใช้กับ pagination)
              $countSQL = "SELECT COUNT(*) AS total FROM type eq";
  ...
  $sql = "SELECT DISTINCT eq.* FROM type eq $orderBy LIMIT $limit OFFSET $offset";
...
?>
	...
	<td align="center"><div class="bootstrap-scope"><a href="edittype.php?eid=<?php echo $row['type_id'];?>">
	  <button type="button" class="btn btn-warning">แก้ไข</button>
	</a>
		<button type="button" class="btn btn-danger" data-bs-toggle="modal" data-bs-target="#exampleModal<?php echo $row['type_id'];?>">
    ลบ
  </button>

  <!-- Modal -->
  <div class="modal fade" id="exampleModal<?php echo $row['type_id'];?>" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <div class="modal-footer">
          ...
          <a href="actions/deltype.php?eid=<?php echo $row['type_id'];?>">
