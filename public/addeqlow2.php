...
$sql = "INSERT INTO `eqlow`(`sumnum`, `name`, `type`, `date1`, `date2`, `money`, `num`, `responsible`, `numstart`, `numend`, `moneynum`, `tranfer`, `price`, `pic`, `num1`, `num2`, `num3`, `num4`) 
...
if (isset($_POST['select_room']) && is_array($_POST['select_room'])) {
    ...
    foreach ($_POST['select_room'] as $room) {
        ...
        if ($quantity) {
            ...
            $sql3 = "INSERT INTO `numroom_low`(`nr`, `room`, `sum_id`) VALUES ('$quantity', '$room', '$sum_id')";
...
$start_suffix = (int)substr($numstart, -6); // ดึง 3 หลักท้ายของหมายเลขเริ่มต้น
$end_suffix = (int)substr($numend, -6); // ดึง 3 หลักท้ายของหมายเลขสุดท้าย
...
for ($i = $start_suffix; $i <= $end_suffix; $i++) {
    ...
    $equipment_no = $prefix . "-" . str_pad($i, 6, '0', STR_PAD_LEFT); // เช่น 511-1500-545-020
